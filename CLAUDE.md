...
A borderless, blurred, always-on-top terminal panel with tabs, themes and global hotkeys. Press ⌃⌥T over whatever you are working in, type a command, press it again. No Dock icon, no menu bar item, no Accessibility permission, no network calls. Current release is `VERSION` in the Makefile (0.1.0), shipped as a DMG.
...
- Corners get a horizontal cursor rather than a diagonal one: AppKit ships no public diagonal resize cursor. Dragging still works in both axes → `Sources/Driftwood/Terminal/ChromeView.swift:resetCursorRects`
