...
A borderless, blurred, always-on-top terminal panel with tabs, themes and global hotkeys. Press ⌃⌥T over whatever you are working in, type a command, press it again. No Dock icon, no menu bar item, no Accessibility permission, no network calls. Current release is `VERSION` in the Makefile (0.4.1), shipped as a DMG.
...
- A second copy at a *different path* runs happily beside the first — same identifier, separate bundles, and LaunchServices only dedupes the same bundle. The new process posts a summon and exits; the running one wins and is never terminated, because that would kill every shell in every tab. **It must not reach `applicationWillTerminate`'s save:** it quits before `AppState.load()`, so saving would write factory defaults over the running instance's `state.json` → `Sources/Driftwood/AppDelegate.swift:handOffToRunningInstance`
- Opening the app while it runs summons the panel, and that handler earns its keep only while the panel is *hidden*. Visible, AppKit's own activation already brings it forward and makes it key; hidden, `orderOut` leaves no window to activate, so Driftwood became frontmost with nothing on screen → `Sources/Driftwood/AppDelegate.swift:applicationShouldHandleReopen`
- A background app's alert takes the keyboard and hands it to the *editor* on dismissal, never back to the panel, because Driftwood is never frontmost. The round trip — focus leaves `focusOwner` for a background app and comes back — is what gets detected; an activation of any app with a Dock icon disarms it, and that, not the 10s timeout, is what stops an ordinary ⌘-Tab from triggering it → `Sources/Driftwood/AppDelegate.swift:applicationActivated`
...
- `scrollbackLines` is applied *after* the terminal view is built, because SwiftTerm 1.15's initializers take no `TerminalOptions` — 500 lines are already allocated by then, and `changeScrollback` resizes the ring in place. 0 means no scrollback, never unlimited, and the cap is finite because the buffer is allocated up front → `Sources/Driftwood/Support/Config.swift:scrollbackLines` and `Sources/Driftwood/Terminal/TerminalSession.swift:init`
...
1. **The focus model.** With an editor frontmost, press ⌃⌥T, type `echo hi`, press Return. The command must run in Driftwood while the editor's title bar stays active. Press ⌃⌥T again; focus must return to the editor untouched. This is the one test the whole design rests on — `showPanel` records `key=` and `frontmost=` in the debug log, so run it with `debug: true` and read the log back. The line to look for is `show: key=true appActive=true frontmost=<your editor>`; read it back rather than trusting the screen.
...
5. Open three tabs, close the middle one, cycle with ⌘⇧] and ⌘⇧[, and jump with ⌘1…⌘3. Some part of the tab strip must stay draggable at every tab count.
6. Change theme, font size and opacity from the menu; quit and relaunch; all three must persist.
7. Drag the panel to a second display, quit, unplug it, relaunch — the panel must come back on a display that exists. **Never run.** No second display was attached when 0.1.0, 0.2.0, 0.3.0 or 0.4.0 was smoke-tested, so this step was skipped rather than passed all four times, and `PanelGeometry.validatedFrame`'s screen-gone branch has only ever been exercised by `make check` against synthetic screen rectangles. Run it before assuming it works. Skip it if there is no second monitor to hand, and record it as skipped rather than passed.
...
9. **Opening the app while it runs.** Hide the panel with ⌃⌥T, then open Driftwood from Finder or Spotlight. The panel must come back with every tab, its scrollback and anything half-typed still there. Repeat with the panel visible: it must come forward and take the keyboard, which is what AppKit's activation does on its own.
10. **A duplicate launch.** With one copy running, `open` a second bundle at another path — `cp -R .build/Driftwood.app /tmp/dw.app` gives you one. The second must exit within a second or two, the first must summon its panel, and `state.json` must be byte-identical afterward (`shasum` it before and after). Verified once this way; the tie-break for two *simultaneous* launches has never been exercised.
11. **`scrollbackLines`, at both ends.** Set it to `50`, relaunch, run `seq 1 500`, and scroll up: the earliest line still reachable must be around 450, not 1. Then set it to `0` and repeat — nothing above the visible screen may be reachable, and a `vim`, `less` or `htop` session must still open, scroll and quit normally, since those draw into the alternate buffer that never had scrollback. `make check` covers the clamping and nothing else; the buffer resize itself is SwiftTerm's and is only exercised here.
12. **Getting the keyboard back after an alert.** With an editor frontmost, press ⌃⌥T and start typing a command. Make something with no Dock icon steal focus — a Little Snitch connection alert is the case this was written for; running `curl` against a host you have no rule for produces one. Answer it. The panel must take the keyboard back within a moment, with the half-typed command still there, and the editor's title bar must stay active throughout. Then check the other half: with the panel focused, ⌘-Tab to another app and back. The panel must **not** take the keyboard, because switching to an app with a Dock icon disarms this. Run both with `debug: true` — `applicationActivated` logs `interrupted by background app …`, `reclaim:` and `focus owner released:`.
13. **`onFocusLoss`, both non-default values.** Set `"onFocusLoss": "hide"`, relaunch, summon the panel, then click into another app — the panel must go away. Summon it again: the tab, its scrollback and anything half-typed must all still be there. Then, with the panel focused, open the palette with ⌃⌥K and right-click for the settings menu; **neither may make the panel disappear**, and that is the case the 200ms delay and `menuIsTracking` exist for. Repeat with `"dim"`, where the same two must not fade it either.
